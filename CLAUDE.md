...
- 机制: push 到 main 后, GitHub Actions 自动把仓库根目录**每个 `.tex`** 编译成
  **同名 `.pdf`** 并用 `[skip ci]` commit 回仓库, 约 2-3 分钟完成。
...
- 新增一份简历 = 新增一个 `.tex`, 零配置; 重命名 = 重命名 `.tex` 并删掉旧 PDF。
...
2. 修改 `.tex`
3. 有本地 TeX 环境时建议先验证:
   `pdflatex -interaction=nonstopmode -halt-on-error -output-directory=build "<文件名>.tex"`
