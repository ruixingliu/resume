...
- 新增一份简历 = 新增一个 `.tex` (英文放 `src/`, 中文放 `src/zh/`), 零配置。
- 重命名 = **只需重命名对应 `.tex`** (本地随便改名, `git add -A` 提交即可); 根目录旧 PDF 由 CI 自动清理
  (CI 会删掉"根目录里找不到对应 `src/**/*.tex`"的孤儿 PDF), 不用手动删。
...
2. 修改 `src/` 里的 `.tex`
3. 有本地 TeX 环境时建议先验证 (先 `mkdir -p src/build`):
   `pdflatex -interaction=nonstopmode -halt-on-error -output-directory=src/build "src/<文件名>.tex"`
   确认 exit 0 且输出 1 page; 没有本地 TeX 就跳过, 交给 CI 验证 (中间产物落在 `src/build/`, 已被忽略)
